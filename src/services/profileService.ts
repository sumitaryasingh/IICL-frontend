...
export const getProfileData = async (
  ...
  if (role === 'admin') {
    try {
      const response = await axioInstance.get<AdminProfileData[]>('/api/admin/profile');
    ...
    try {
      const response = await axioInstance.get<FranchiseData[]>(`/api/profile/franchise-profile/${franchiseId}`);
